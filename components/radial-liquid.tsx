...
export const RadialLiquid = ({
  ...
  return (
    <div
      ref={containerRef}
      className={`radial-liquid ${className || ""}`}
      style={{
        width: widthStyle,
        height: heightStyle,
    ...
    >
      <canvas ref={canvasRef} className="radial-liquid__canvas" />
      {children && <div className="radial-liquid__content">{children}</div>}
    </div>
  );
};
